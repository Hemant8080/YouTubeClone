  ...
  return (
    <>
      <div
        className={`${
          mobileShow
            ? "fixed top-0 left-0 transition-all duration-300 bg-white z-40 h-screen w-[70%] sm:w-[30%]"
            : "hidden w-[18%] h-[calc(100vh-6.625rem)] "
        }  xl:static xl:block  px-2 lg:px-6  overflow-y-scroll overflow-x-hidden scrollbar-thin scrollbar-none`}
      >
        {/* Home */}
          ...
          {sidebarItems.map((group, index) => {
            return (
              <div key={index}>
                ...
                {group.groupItem.map((item, index) => {
                  return (
                    <div
                      key={index}
